...
class Block:
    ...
    def __str__(self) -> str:
        ...
        if len(self.children) == 0:
            ...
            indents = '\t' * self.level
            result = (
                f'{indents}Parent: pos={self.position},' 
                f'size={self.size}, level={self.level}'
            )
    ...
    def _update_children_positions(self, position: tuple[int, int]) -> None:
        ...
        if len(self.children) == 4:
            new_positions = self.children_positions()
    ...
    def smash(self) -> bool:
        ...
        for i in range(4):
            ...
            new_block = Block(
                new_positions[i], child_size, child_colour, self.level + 1,
                self.max_depth
            )
            self.children.append(new_block)

            if random.random() < math.exp(-0.25 * new_block.level):
                new_block.smash()
    ...
    def rotate(self, direction: int) -> bool:
        ...
        if len(self.children) != 4 or len(self.children) == 0:
            return False

        else:
            if direction == ROT_CW:     # Rotate clockwise
                lu, ll, ru, rl = (self.children[0], self.children[2],
                                  self.children[1], self.children[3])
                self.children = [ru, ll, rl, lu]
                self._update_children_positions(self.position)
    ...
    def combine(self) -> bool:
        ...
        if self.level != (self.max_depth - 1) and len(self.children) == 0:
            return False
        colour_counts = {colour: 0 for colour in COLOUR_LIST}
        for child in self.children:
        ...
        majority_colour = max(colour_counts, key=colour_counts.get)
