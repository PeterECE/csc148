...
class CustomerArrival(Event):
    ...
    def do(self, store: GroceryStore) -> list[Event]:
        try:
            ...
            if len(store.lines[checkout_line]) == 1:
...
class CheckoutStarted(Event):
    ...
    def do(self, store: GroceryStore) -> list[Event]:
        if not store.lines[self.line_number]:
            return []
        next_checkout_time = store.next_checkout_time(self.line_number)
        customer = store.first_in_line(self.line_number)
        return [CheckoutCompleted
                (self.timestamp
                 + next_checkout_time, self.line_number, customer)]
...
class CheckoutCompleted(Event):
    ...
    def do(self, store: GroceryStore) -> list[Event]:
        remaining = store.remove_front_customer(self.line_number)
        if remaining == 0:
            return []
...
class CloseLine(Event):
    ...
    def do(self, store: GroceryStore) -> list[Event]:
        ...
        for customer in customers[::-1]:
            event_list.append(CustomerArrival(next_timestamp, customer))
            next_timestamp += 1
        return event_list
...
def create_event_list(event_file: TextIO) -> list[Event]:
    ...
    for line in event_file:
        ...
        if contents[1] == 'Arrive':
            customer_name = contents[2]
            items = []
            for i in range(3, len(contents), 2):
                items.append(Item(contents[i], int(contents[i + 1])))
            customer = Customer(customer_name, items)
            event_list.append(CustomerArrival(timestamp, customer))
        elif contents[1] == 'Close':
            line_number = int(contents[2])
            event_list.append(CloseLine(timestamp, line_number))
    return event_list
